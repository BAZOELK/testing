...
NC="\e[0m"
COLOR1="$(cat /etc/tester/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/tester/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"

APIGIT=$(cat /etc/tester/github/api)
EMAILGIT=$(cat /etc/tester/github/email)
USERGIT=$(cat /etc/tester/github/username)
...
if [[ -f /etc/tester/github/api && -f /etc/tester/github/email && /etc/tester/github/username ]]; then
   rec="OK"
else
    mkdir /etc/tester/github > /dev/null 2>&1
fi
...
sleep 2
echo "$EMAIL1" > /etc/tester/github/email
echo "$USERNAME1" > /etc/tester/github/username
echo "$API1" > /etc/tester/github/api
echo "ON" > /etc/tester/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/tester/github/email
rm -f /etc/tester/github/username
rm -f /etc/tester/github/api
rm -f /etc/tester/github/gitstat
echo "OFF" > /etc/tester/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
GITREQ=/etc/tester/github/gitstat
if [ -f "$GITREQ" ]; then
    cekk="ok"
else 
    mkdir /etc/tester/github
    touch /etc/tester/github/gitstat
    echo "OFF" > /etc/tester/github/gitstat
fi

stst1=$(cat /etc/tester/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1 ${NC} ${COLBG1}               • REGISTER IPVPS •              ${NC} $COLOR1 $NC"
...
stst=$(cat /etc/tester/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
